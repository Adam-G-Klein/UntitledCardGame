...
public class ShopManager : GenericSingleton<ShopManager>, IEncounterBuilder
{
    ...
    ShopPhase currentDemoShopPhase;
    int shopIndex = 0; // only used for demo shops, to track which shop we're on for the purposes of demo dialogue and stuff
    ...
    public void BuildShopEncounter(ShopEncounter shopEncounter) {
        ...
        if(gameState.BuildTypeDemoOrConvention())
        {
            ...
            currentStaticShopPoolEncounter = shopEncounter.shopData.staticShopEncounters.shopPoolEncounter;
        ...
        */
        ...
        if (gameState.BuildTypeDemoOrConvention())
        {
            if(shopEncounter.shopData.shopMode == ShopMode.StaticChooseNDemo)
            {
                StartCoroutine(RunShopDialogueStep(ShopDemoEvent.EnterShop));
            }
 ...
 // Used in the SD26 GDC tutorial cinematic
    ...
    private void incrementCardGroupIndex()
    {
        ...
        currentCardGroupIndex = currentCardGroupIndex % currentStaticShopPoolEncounter.cardGroups.Count;
    }

    private void incrementRatGroupIndex()
    {
        currentRatGroupIndex += 1;
        currentRatGroupIndex = currentRatGroupIndex % currentStaticShopPoolEncounter.ratGroups.Count;
    }
    ...
    public void ProcessCompanionBuyRequest(ShopItemView shopItemView, CompanionInShopWithPrice companionInShop) {
        ...
        if ((shopEncounter.shopData.shopMode != ShopMode.StaticChooseNDemo && gameState.playerData.GetValue().gold >= companionInShop.price) || (shopEncounter.shopData.shopMode == ShopMode.StaticChooseNDemo && numRatsBoughtThisShop < shopEncounter.shopData.numRatsBuyPerShop))
        {
            ...
            // If no upgrade would occur, check if we have space on the team.
            if (upgradeInfo == null && gameState.companions.activeCompanions.Count + shopViewController.GetBlockedCompanionSlots() == 5 && gameState.companions.benchedCompanions.Count == availableBenchSlots)
            {
    ...
    public void EarnUpgradeIncrement()
    {
        ...
        if (didUpgrade) {
            ...
            shopViewController.SetShopUpgradePrice(shopLevel.upgradeIncrementCost);
            if (timesRerolledThisShop < shopLevel.numCardRemovalsAllowed)
            {
    ...
    private void rerollShop()
    {
        ...
        if(gameState.BuildTypeDemoOrConvention())
        {
            ...
            staticRats = new StaticCompanionTypeGroup
            {
