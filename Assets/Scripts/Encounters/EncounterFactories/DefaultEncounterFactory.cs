...
public class DefaultEncounterFactory 
{
    ...
    public void generateEncounter(DefaultEncounter encounter) {
        ...
        CompanionManager companionManager = managersGameObject.GetComponent<CompanionManager>();
        ...
        generateEnemies(encounter, enemyManager, prefabStore);
        generatePlayer(player);
        Debug.Log("companion manager" + companionManager);
        Debug.Log("prefabStore " + prefabStore);
        generateCompanions(companionManager, prefabStore);
    ...
    private void generateEnemies(
        ...
        LocationStore locStore = GameObject.FindGameObjectWithTag("EnemyStore")
            .GetComponent<LocationStore>();
    ...
    private void generateCompanions(
            CompanionManager companionManager,
            PrefabStore prefabStore) {
        LocationStore locStore = GameObject.FindGameObjectWithTag("CompanionStore")
            .GetComponent<LocationStore>();
        List<Companion> companions = companionManager.getActiveCompanions();
        List<Vector2> locList = locStore.getLocs(companions.Count);
        Companion companion;
        for (int i = 0; i < companions.Count; i += 1) {
            companion = companions[i];
            companion.buildCompanion(
                prefabStore.getPrefabByName(companion.getPrefabName()),
                locList[i]);
        }
    }
