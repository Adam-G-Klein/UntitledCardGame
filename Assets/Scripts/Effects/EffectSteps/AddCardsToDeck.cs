...
public class AddCardsToDeck : EffectStep {
    ...
    public override IEnumerator invoke(EffectDocument document) {
        ...
        if (entities.Count == 0) {
            Debug.LogError("AddCardsToDeck Effect: No valid inputs for adding card to deck");
            yield return null;
        }
        ...
        if (getCardsFromKey) {
            if (!document.cardMap.containsValueWithKey(inputCardsKey)) {
                Debug.LogError("AddCardsToDeck Effect: No valid card input but GetCardsFromKey was set");
                yield return null;
            }
